...
def gen_heatmap(vis_path, num_img, model, inputs, labels, original_image):
    heatmap = make_heatmap(model, inputs, labels, 33)
    # plot heatmap
    ...
    # save fig
    fig = plt.gcf()
    fig.savefig(osp.join(vis_path, num_img, 'ori.png'), format='png', bbox_inches='tight', transparent=True, dpi=600)
    plt.close()
...
def make_heatmap(model, image, target, patchsize):
    """
