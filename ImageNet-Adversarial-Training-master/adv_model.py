...
class AdaAttacker():
    ...
    def attack(self, image_clean, label, model_func):
        ...
        def one_step_attack(i, adv, m, exp_avg, exp_avg_sq, max_exp_avg_sq):
            ...
            adv = tf.clip_by_value(adv - tf.sign(m) * tf.minimum(self.epsilon, self.step_size/ bias_correction1), lower_bound, upper_bound)
        ...
        init_momentum = init_exp_avg = init_exp_avg_sq = init_max_exp_avg_sq = tf.zeros_like(start_adv)

        with tf.name_scope('attack_loop'):
            _, adv_final, _, _, _, _ = tf.while_loop(cond, one_step_attack, (tf.zeros([]), start_adv, init_momentum, init_exp_avg, init_exp_avg_sq, init_max_exp_avg_sq), back_prop=False, maximum_iterations=self.num_iter)

        return adv_final, target_label


class MomentumAttacker():
    """
    ...
    def attack(self, image_clean, label, model_func):
        ...
        def one_step_attack(i, adv, m):
            ...
            red_ind = list(range(1, len(g.get_shape())))
            avoid_zero_div = tf.cast(1e-12, g.dtype)
            g = g / tf.maximum(avoid_zero_div, tf.reduce_mean(tf.abs(g), red_ind, keepdims=True))
            
            m = self.decay_factor * m + g
            """
            Feature Denoising, Sec 5:
            ...
            """
            adv = tf.clip_by_value(adv - tf.sign(m) * self.step_size, lower_bound, upper_bound)
        ...
        start_from_noise_index = tf.cast(tf.greater(tf.random_uniform(shape=[]), self.prob_start_from_clean), tf.float32)
        start_adv = image_clean + start_from_noise_index * init_start
        init_momentum = tf.zeros_like(start_adv)

        with tf.name_scope('attack_loop'):
            _, adv_final, _ = tf.while_loop(cond, one_step_attack, (tf.zeros([]), start_adv, init_momentum), back_prop=False, maximum_iterations=self.num_iter)
