...
import nets
from adv_model import NoOpAttacker, PGDAttacker, MomentumAttacker, AdaAttacker
...
def do_train(model):
    ...
    if not args.fake:
        ...
        elif args.attacker == 'Ada':
            Attacker = AdaAttacker

        add_eval_callback('eval-clean', NoOpAttacker(), lambda e: True)
        add_eval_callback('eval-10step', Attacker(10, args.attack_epsilon, args.attack_step_size), lambda e: True)
        add_eval_callback('eval-50step', Attacker(50, args.attack_epsilon, args.attack_step_size), lambda e: e % 20 == 0)
        add_eval_callback('eval-100step', Attacker(100, args.attack_epsilon, args.attack_step_size), lambda e: e % 10 == 0 or e > max_epoch - 5)
        for k in [20, 30, 40, 60, 70, 80, 90]:
            add_eval_callback('eval-{}step'.format(k), Attacker(k, args.attack_epsilon, args.attack_step_size), lambda e: False)
...
if __name__ == '__main__':
    ...
    # attacker flags:
    parser.add_argument('--attack-iter', help='Adversarial attack iteration', type=int, default=30)
    parser.add_argument('--attack-epsilon', help='Adversarial attack maximal perturbation', type=float, default=16.0)
    parser.add_argument('--attack-step-size', help='Adversarial attack step size', type=float, default=1.0)
    parser.add_argument('--use-fp16xla', help='Optimize PGD with fp16+XLA in training or evaluation. '
    ...
    parser.add_argument('--attacker', help='Name of attacker defined in adv_model.py', default='PGD')
    ...
    if args.attack_iter == 0 or args.eval_directory:
        ...
        if args.attacker == 'PGD':
            ...
            attacker = MomentumAttacker(args.attack_iter, args.attack_epsilon, args.attack_step_size, prob_start_from_clean=0.2 if not args.eval else 0.0)
        elif args.attacker == 'Ada':
            attacker = AdaAttacker(args.attack_iter, args.attack_epsilon, args.attack_step_size, prob_start_from_clean=0.2 if not args.eval else 0.0)
